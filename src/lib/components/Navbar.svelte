<script lang="ts">
	...
	let statusElemet: HTMLDivElement;
	onMount(async () => {
...
<nav class="align-center flex justify-between bg-primary-600 p-4 text-white">
	...
	<div class="flex items-center" bind:this={statusElemet}>API status: ⏳</div>
	<div class="flex items-center">
		{#if $page.data.session?.user}
			<p class="pr-2">{$page.data.session.user.name}</p>
			<button class="rounded bg-secondary-600 px-4 py-2 text-white transition duration-150 ease-in-out hover:bg-secondary-700" on:click={signOut}
				>Sign out</button
			>
		{:else}
			<button
